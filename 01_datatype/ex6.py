...
# a[0] = "J"                          # 문자열 내용 수정이 가능할까?  -> error 뜸
print(type(a[0]))

b = [10,20,30]                        # list는 내용 수정할 수 있음
b[0] = 40
print(b)

# b.__setitem__  # 이게 있어서 list를 수정할 수 있음

# 문자열 메소드로 변경해도 원본은 그대로, 새로운 문자열 객체가 생성됨
b = a.upper()  # 새로 만들어서 b에 넣음

print(a, b)
print(id(a), id(b))  # 두개는 다 다른거임
print(a is b)  # 같은 객체인가?

# 문자열 연결 시 + 연산도 새로운 문자열 객체를 만듦
a = "Hello"
print(a, id(a))
a += " Python"  # 새로운 문자열이 만들어짐
print(a, id(a))
...
# 확장 언패킹
a, *b, c = s         # P만 a에 들어가고 나머지는 b에 (b는 list)(c는 마지막 단어 n을 받음)
print(a, b, c)
...
print(id(a[0]),id(a[0]))            # 같은 것을 여러번 만들지 않음

# 슬라이싱 (start:end-1:step)
print(a[0:2])                       # Py 출력 / step은 생략 가능
...
print(a[2:100])                     # thon 출력 -> 그냥 문장의 끝까지 출력됨
...
print(f"{s1[0]}학년 {s1[1]}반 {int(s1[2]+s1[3])}번")                  # ✅ "1학년 3반 17번" 출력

# 2️⃣ 학번을 슬라이싱만을 이용해서 학년, 반, 번호로 잘라내기
print(f"{s[0:1]}학년 {s[1:2]}반 {int(s[2:4])}번")                  # ✅ "1학년 3반 1번" 출력
print(f"{s1[0:1]}학년 {s1[1:2]}반 {int(s1[2:4])}번")                  # ✅ "1학년 3반 17번" 출력